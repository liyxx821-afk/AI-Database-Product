...
| Smoke order | `pnpm smoke:p0-core` 必须先于 `pnpm smoke:p0-desktop-runtime`，`pnpm smoke:p0-desktop-runtime` 必须先于 `pnpm smoke:p0-packaged-runtime`，`pnpm smoke:p0-packaged-runtime` 必须先于 `pnpm smoke:p0-i18n-settings`，`pnpm smoke:p0-i18n-settings` 必须先于 `pnpm smoke:p0-file`，`pnpm smoke:p0-file` 必须先于 `pnpm smoke:p0-parse`，`pnpm smoke:p0-parse` 必须先于 `pnpm smoke:p0-ku`，`pnpm smoke:p0-ku` 必须先于 `pnpm smoke:p0-space-tag-metadata`，`pnpm smoke:p0-space-tag-metadata` 必须先于 `pnpm smoke:p0-text-to-sql`，`pnpm smoke:p0-text-to-sql` 必须先于 `pnpm smoke:p0-knowledge-export`，`pnpm smoke:p0-knowledge-export` 必须先于 `pnpm smoke:p0-knowledge-export-history`，`pnpm smoke:p0-knowledge-export-history` 必须先于 `pnpm smoke:p0-search-ask`，`pnpm smoke:p0-search-ask` 必须先于 `pnpm smoke:p0-citation-detail`，`pnpm smoke:p0-citation-detail` 必须先于 `pnpm smoke:p0-citation-focus`，`pnpm smoke:p0-citation-focus` 必须先于 `pnpm smoke:p0-citation-annotate-compare`，`pnpm smoke:p0-citation-annotate-compare` 必须先于 `pnpm smoke:p0-batch-actions`，`pnpm smoke:p0-batch-actions` 必须先于 `pnpm smoke:p0-feedback-memory`，`pnpm smoke:p0-feedback-memory` 必须先于 `pnpm smoke:p0-feedback-diagnostics`，`pnpm smoke:p0-feedback-diagnostics` 必须先于 `pnpm smoke:p0-feedback-export`，`pnpm smoke:p0-feedback-export` 必须先于 `pnpm smoke:p0-feedback-filters-history`，`pnpm smoke:p0-feedback-filters-history` 必须先于 `pnpm smoke:p0-z0a` 通过 |
...
### 2.13 D-113 Citation Annotation / Evidence Compare 测试口径

D-113 后，Citation Detail 增加本地批注和同一 Evidence Pack 内 evidence compare。测试目标是证明批注可持久化、compare 只读、copy-safe payload 不泄漏 source text / token / path，且二者不影响 Evidence Pack、KU、Source、AIAnswer、Memory 或 ranking。

| 测试项 | 期望 |
|---|---|
| annotation auth | 无 local token 访问 annotation / compare API 被拒绝 |
| annotation CRUD | confirmed KU 检索后，可为 evidence item 创建、读取、更新、删除批注 |
| annotation binding | annotation item 必须属于目标 Evidence Pack；跨 pack item 返回 `evidence_item_not_in_pack` |
| annotation validation | `annotation_type` 仅支持 note / question / risk / follow_up，content 不能为空 |
| compare scope | compare 只接受同一 pack 内 2-3 条 evidence item |
| compare detail | compare 返回 source/chunk/KU/rank/trace path/copy-safe summary |
| no mutation | annotation / compare 不修改 evidence pack、evidence items、KU、Source、AIAnswer、Memory 或 ranking |
| UI interaction | `/search` 与 `/ask` Citation Detail 面板可对 focused item 批注、编辑/删除，并选择 2-3 条 evidence items 展示对比 |

`smoke:p0-citation-annotate-compare` 固定链路：

```text
create multiple text imports
→ confirm KUs
→ retrieval preview
→ open evidence pack detail
...
### 2.12E D-119 Text-to-SQL Template / Structured Query Preview 测试口径

D-119 后，结构化查询预览只验证规则模板与 SQL 安全边界，不验证真实 Text-to-SQL 模型质量。测试目标是证明用户自然语言 query 不会变成自由 SQL，且 preview 可复盘 SQL trace、参数、结果行和 degraded provider/fallback 状态。

| 测试项 | 期望 |
|---|---|
| text-to-sql auth | 无 local token 访问 `/api/text-to-sql/preview` 被拒绝 |
| confirmed KU lookup | confirmed KU 可返回；`pending_review` KU 默认不进入结果 |
| organization filters | `project_id`、`folder_id`、`tag_ids` 对 KU lookup / source inventory 生效 |
| readonly safety | 恶意 query 只作为参数处理，模板仍是单条 `SELECT` |
| retrieval log | 每次 preview 写入 `retrieval_logs.query_intent=structured_query_preview` |
| no leakage | Response 不包含 local token、SQLite path、app data path、完整本地路径或 provider secret |

`smoke:p0-text-to-sql` 固定链路：

```text
text import
→ bind folder/tag
→ pending preview excludes candidate KU
→ confirm KU
→ confirmed KU lookup
→ source inventory
→ retrieval preview
→ evidence history
→ malicious query safety check
```

### 2.12C D-118 Citation / Organization / Export Batch Actions 测试口径

D-118 后，批量操作只复用既有对象，不新增表、不写 ranking signal。测试目标是证明批量 Source/KU organization、批量 citation annotation 和 KU 选择导出都可验证，且无 token / path 泄露。

| 测试项 | 期望 |
|---|---|
| batch auth | 批量组织和批量批注 API 无 local token 被拒绝 |
| source batch | 1-50 个 Source 可批量替换 folder/tags，并同步派生 KU 的 folder mirror 与 source-assignment tags |
| KU batch | 1-50 个 KU 可批量替换自身 folder/user tags，不修改 Source 原始记录 |
| annotation batch | 同一 Evidence Pack 内 1-20 个 evidence item 可创建同内容批注；跨 pack item 返回 `evidence_item_not_in_pack` |
| selected export | `knowledge_unit_ids` 选择导出只包含指定 confirmed KU；默认不导出 pending_review |
| no mutation | 批量批注不修改 Evidence Pack、Evidence Item、KU、Source、AIAnswer、Memory 或 ranking |

`smoke:p0-batch-actions` 固定链路：

```text
create multiple text imports
→ batch bind Source organization
→ confirm KUs
→ batch bind selected KU organization
→ retrieval preview with organization filters
→ batch annotate selected evidence items
→ selected KU Markdown / JSON export
→ verify no token/path leakage and invalid binding checks
```
...
### 2.13 D-115 Knowledge Unit / Project Export 测试口径

D-115 后，知识资产导出必须复用 D-114 的 Project / Folder / Tag 过滤，不新增数据库表，不写本地用户路径，不泄露 token、SQLite path、app data path 或完整本地路径。

| 测试项 | 期望 |
|---|---|
| export auth | 无 local token 访问 `/api/exports/knowledge-units` 与 `/api/exports/project` 被拒绝 |
| Markdown export | confirmed KU 可导出 Markdown，YAML frontmatter 包含 type / tags / status / source / chunk citation |
| JSON export | JSON 包含 manifest、project、folders、tags、knowledge_units，并按请求包含 chunks / sources |
| project ZIP | ZIP 文件清单固定为 `manifest.json`、`knowledge-units.json`、`tags.json`、`folders.json`、`sources.json`、`chunks.json`、`README.md` |
| organization filters | `folder_id` / `tag_ids` / `knowledge_unit_ids` 只导出匹配 KU |
| pending boundary | 默认不导出 `pending_review` KU；只有显式 `include_pending_review=true` 才允许进入导出 |
| redaction | 导出内容不得包含 local token、SQLite path、app data path、完整本地路径或原始二进制文件 |
| invalid ids | invalid project/folder/tag/KU id 返回现有 error envelope |

`smoke:p0-knowledge-export` 固定链路：

```text
text import
→ confirm KU
→ create folder/tag
→ bind KU through Source organization
→ export filtered Markdown
→ export JSON
→ export Project ZIP
→ verify manifest/files/content
→ verify no path/token leakage
```

### 2.13 D-116 Knowledge Export History / Replay 测试口径

D-116 后，知识资产导出历史必须复用 D-115 export envelope 和 D-106/D-111 的 `config.json` 原子持久化方式，不新增 SQLite 表，不保存导出正文。

| 测试项 | 期望 |
|---|---|
| history auth | 无 local token 访问 `/api/exports/history` 被拒绝 |
| history append | 每次成功 KU Markdown / JSON / Project ZIP 导出后写入最近 20 条 metadata |
| content hash | text export 使用 UTF-8 content sha256；ZIP 使用原始 archive bytes sha256 |
| history redaction | history 不保存 `content`、`content_base64`、source text、KU 正文、token、DB path 或完整本地路径 |
| delete history | 删除单条 history 后列表不再返回该记录，且不影响 KU、Source、Chunk、Project、Tag |
| renderer replay | `/outputs` Knowledge Export History 可刷新、删除记录，并把历史 filters 套用回当前导出控件 |

`smoke:p0-knowledge-export-history` 固定链路：
...
→ export JSON
→ export Project ZIP
→ read knowledge export history
→ verify content_sha256 and filters
→ delete one history record
→ verify no content/path/token leakage
```
