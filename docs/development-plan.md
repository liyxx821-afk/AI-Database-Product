...
1. 保持 `smoke:p0-core` → `smoke:p0-desktop-runtime` → `smoke:p0-i18n-settings` → `smoke:p0-file` → `smoke:p0-parse` → `smoke:p0-ku` → `smoke:p0-search-ask` → `smoke:p0-citation-detail` → `smoke:p0-feedback-memory` → `smoke:p0-feedback-diagnostics` → `smoke:p0-z0a` 的本地 gate，避免后续 UI settings、AI / Retrieval / Feedback 改动破坏 runtime、Electron bridge、OpenAPI、SQLite、upload、parse、KU/Review、evidence contract、citation detail replay、memory review 和 feedback replay。
2. 评估 feedback 诊断导出 / 深层过滤、Citation detail 深层交互，或更接近真实 packaged sidecar 的构建 / 分发 spike；Memory 仍不得绕过 Review 进入 confirmed。
