...
- **文档同步完成**：已同步 `docs/git-management.md` v0.2、`docs/project-background-brief.md` v0.6、`docs/product-architecture.md` v0.14、`docs/mvp-scope.md` v0.13、`docs/architecture-design-plan.md` v1.3、`docs/data-model.md` v0.20-draft、`docs/api-design.md` v0.18-draft、`docs/api-implementation-plan.md` v0.20-draft、`docs/p0a-execution-plan.md` v0.21、`docs/technical-stack-and-prototype-plan.md` v0.23、`docs/desktop-architecture.md` v0.6、`docs/ai-provider-architecture.md` v0.14、`docs/testing-strategy.md` v0.21、`docs/rag-pipeline.md` v0.8-draft、`docs/knowledge-invocation-system-design-plan.md` v0.10、`docs/error-handling-and-observability.md` v0.9、`docs/text-to-sql.md` v0.4-draft、README 与进度记录。
...
1. 按 `docs/p0a-execution-plan.md` v0.21 创建 P0-Core 工程骨架，先落地 `apps/desktop-main`、`apps/desktop-preload`、`apps/renderer`、`apps/api`、`packages/api-types`、`packages/runtime-contracts` 和 `packages/shared-config` 的职责边界。
2. 先实现 Electron Main + secure preload bridge、FastAPI `/api/health` + local token middleware、app data dir + SQLite init、migration skeleton + backup hook、`GET /api/system/runtime`、renderer shell + runtime status bar、OpenAPI export + generated TS types。
3. 先通过 `pnpm smoke:p0-core`，再开发 `pnpm smoke:p0-z0a`；`smoke:p0-core` 未通过前禁止 upload parsing UI、RAG UI、真实 Provider 接入、OCR/ASR、图谱功能和面向用户的 Chat。
4. `smoke:p0-core` 通过后，再按 `docs/data-model.md` v0.20-draft 冻结 P0-Z0a 第一批 migration：local_user / project / text-import / inspect / parse / chunk / KU review / embed job / retrieval log / Evidence Pack / evidence-only answer；D-092 的 `trace_id` 与 Evidence Pack `failure_type` 进入首批可追踪字段。
5. 按 `docs/api-design.md` v0.18-draft 与 `docs/api-implementation-plan.md` v0.20-draft 生成 OpenAPI / route skeleton；D-094 首批 API 只阻塞 `/api/health`、`/api/system/runtime`、`/api/system/status`、`/api/system/diagnostics:export` 和 `/api/auth/status`。
