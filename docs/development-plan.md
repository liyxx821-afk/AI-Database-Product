...
1. 保持 `smoke:p0-core` → `smoke:p0-desktop-runtime` → `smoke:p0-i18n-settings` → `smoke:p0-file` → `smoke:p0-parse` → `smoke:p0-ku` → `smoke:p0-search-ask` → `smoke:p0-citation-detail` → `smoke:p0-citation-focus` → `smoke:p0-feedback-memory` → `smoke:p0-feedback-diagnostics` → `smoke:p0-feedback-export` → `smoke:p0-feedback-filters-history` → `smoke:p0-z0a` 的本地 gate，避免后续 UI settings、AI / Retrieval / Feedback 改动破坏 runtime、Electron bridge、OpenAPI、SQLite、upload、parse、KU/Review、evidence contract、citation detail replay、citation focus、memory review、feedback replay、导出脱敏和 export history 边界。
2. 下一阶段可评估 Citation detail 批注 / 证据对比、导出历史复用体验、或更接近真实 packaged sidecar 的构建 / 分发 spike；Memory 仍不得绕过 Review 进入 confirmed。
3. 若继续桌面打包方向，下一步应验证正式静态 renderer 载入方式、日志目录、资源路径和安装器前的 sidecar artifact，但仍不引入真实签名、自动更新或正式品牌资源。
