...
版本：v0.61
日期：2026-05-17  
状态：已完成完整 P0 架构升级、技术选型细化、File Inspection 架构细化、切片前准备层、结构化整理检查门、知识切片质量闭环、安全运维横切层、实现前契约去重、检查门映射收敛、P0-Z0a/Z0b 竖切、ProcessingJob 契约收紧、切片执行 profile、AI 结构化整理 profile、D-077 实现边界收敛、D-079 结构化整理与存储映射收敛、D-080 知识调用 / AI 智能体 / 前端交互收敛、D-081/D-082/D-083 实现前边界修正、D-084 跨文档同步、D-085 Z0a 调用锚点与反馈 / citation 边界修正、D-086 后续 Agent 项目背景说明书、D-087 本地 Git 管理基础、D-088 README 对齐后的技术栈优化、D-089 GitHub private repo 上传、D-090 技术栈执行优化、D-091 技术栈工程化验收门槛、D-092 代码骨架前置契约优化、D-093 桌面运行时硬化、D-094 P0-Core 桌面工程骨架开工契约、D-095 架构审查实现验收强化、D-096 分布式开发计划、D-097 分布式开发计划优化、D-098 前端页面信息架构与工作台契约收敛、D-099 前后端交付边界与分布式开发计划强化、D-100 P0 代码阶段首轮实现、D-101 P0-File 上传与 File Inspection Z0a、D-102 Parser Router / Source / Chunk Z0a、D-103 Candidate KU / Review / fallback embedding Z0a、D-104 Retrieval Preview / Search-Ask Integration Z0a、D-105 Citation Detail / Evidence Pack Replay Z0b-lite、D-106 UI i18n / Bilingual Settings Z0b-lite，以及 D-107 Feedback Events / Memory Draft Review Z0b-lite（D-066 - D-107）

## 1. 当前阶段状态

本项目当前处于 **P0 Feedback Events / Memory Draft Review Z0b-lite 代码阶段**。D-100 已从文档-only 进入可运行 monorepo；D-101 已补齐文件上传和 File Inspection Z0a；D-102 已将已检查的文本类文件显式解析为 Source / Chunk；D-103 已将 parsed Source / Chunk 接入 Candidate KU、Review Task 和 `mock_fixed_384` fallback embedding；D-104 已将 confirmed KU 接入 Retrieval Preview、Evidence Pack detail、`/search`、`/ask` 和 `smoke:p0-search-ask`；D-105 已将 Evidence Pack detail 扩展为 Citation Detail replay，并在 `/search` 与 `/ask` 内嵌复盘面板和 `smoke:p0-citation-detail`；D-106 已将八页低保真工作台核心 UI 文案改为默认中文、可切换英文，并通过 `/api/settings` 将语言偏好持久化到 app data `config.json`；D-107 已补齐 append-only feedback events、pending-review Memory Draft、memory review confirm/ignore、`/ask` 反馈与保存入口、`/outputs` 草稿摘要和 `smoke:p0-feedback-memory`。
...
1. 保持 `smoke:p0-core` → `smoke:p0-i18n-settings` → `smoke:p0-file` → `smoke:p0-parse` → `smoke:p0-ku` → `smoke:p0-search-ask` → `smoke:p0-citation-detail` → `smoke:p0-feedback-memory` → `smoke:p0-z0a` 的本地 gate，避免后续 UI settings、AI / Retrieval / Feedback 改动破坏 runtime、OpenAPI、SQLite、upload、parse、KU/Review、evidence contract、citation detail replay 和 memory review。
2. 评估 Citation detail 深层交互、feedback 诊断视图或 pseudo-packaged sidecar / Electron GUI smoke；Memory 仍不得绕过 Review 进入 confirmed。
