...
1. 保持 `smoke:p0-core` → `smoke:p0-i18n-settings` → `smoke:p0-file` → `smoke:p0-parse` → `smoke:p0-ku` → `smoke:p0-search-ask` → `smoke:p0-citation-detail` → `smoke:p0-feedback-memory` → `smoke:p0-feedback-diagnostics` → `smoke:p0-z0a` 的本地 gate，避免后续 UI settings、AI / Retrieval / Feedback 改动破坏 runtime、OpenAPI、SQLite、upload、parse、KU/Review、evidence contract、citation detail replay、memory review 和 feedback replay。
2. 评估 Citation detail 深层交互、feedback 诊断深层过滤 / 导出或 pseudo-packaged sidecar / Electron GUI smoke；Memory 仍不得绕过 Review 进入 confirmed。
3. 补充 Electron GUI smoke 或 pseudo-packaged sidecar spike，处理 pnpm 安装时 Electron / esbuild build scripts 被忽略后的真实启动验证。
