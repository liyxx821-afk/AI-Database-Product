...
- **文档同步完成**：已同步 `docs/git-management.md` v0.2、`docs/project-background-brief.md` v0.5、`docs/product-architecture.md` v0.14、`docs/mvp-scope.md` v0.13、`docs/architecture-design-plan.md` v1.3、`docs/data-model.md` v0.20-draft、`docs/api-design.md` v0.18-draft、`docs/api-implementation-plan.md` v0.19-draft、`docs/p0a-execution-plan.md` v0.20、`docs/technical-stack-and-prototype-plan.md` v0.22、`docs/ai-provider-architecture.md` v0.14、`docs/testing-strategy.md` v0.20、`docs/rag-pipeline.md` v0.8-draft、`docs/knowledge-invocation-system-design-plan.md` v0.10、`docs/error-handling-and-observability.md` v0.8、`docs/text-to-sql.md` v0.4-draft、README 与进度记录。
...
1. 按 `docs/data-model.md` v0.20-draft 冻结 P0-Z0a 第一批 migration：local_user / project / upload-or-text-import / inspect / parse / chunk / KU review / embed job / retrieval log / Evidence Pack / evidence-only answer；AI 结构化整理 profile 和 D-080-D085 调用 profile 继续复用 metadata / processing events / review tasks / retrieval logs，D-092 的 `trace_id` 与 Evidence Pack `failure_type` 进入首批可追踪字段。
2. 按 `docs/api-design.md` v0.18-draft 与 `docs/api-implementation-plan.md` v0.19-draft 生成 OpenAPI / route skeleton；D-080-D085 只影响 retrieval / RAG response schema 示例和 Z0a/Z2 持久化边界，不新增 endpoint；D-092 要求 OpenAPI 导出的 TypeScript 类型参与前端 typecheck；D-093 要求 sidecar local auth、runtime health、worker status 和脱敏诊断约束进入 API middleware / system API。
3. 优先实现 `JobRepository`、`ProcessingEventService`、ProcessingJob 状态机、worker heartbeat 和 active job 幂等，再接 upload / inspect / parse。
4. 为 sensitive evidence 增加 `sensitive_access_grants` DTO / repository / contract tests，覆盖过期、撤销、scope 不匹配和二次复用。
5. 按 `docs/p0a-execution-plan.md` v0.20 创建 P0-Core 工程骨架，并在 W1 即补上合同测试和 P0 smoke test；首批只阻塞 `uv` 的 `core + dev` 依赖组、`pnpm` workspace、OpenAPI 类型生成、Zustand store slices、typed fetch、sidecar lifecycle / 打包 spike、sidecar local auth、Provider manifest / lazy-load、sqlite-vec capability probe、SQLite WAL / backup / integrity check、SQLite 1K/10K baseline、worker heartbeat、runtime status bar、trace chain 和 P0-Z0a evidence-first smoke。
