...
2. 下一阶段可继续收敛真实安装器 / PyInstaller sidecar binary spike，或进入第二轮 UI 设计稿替换前的交互 polish；Memory 仍不得绕过 Review 进入 confirmed。
