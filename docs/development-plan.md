...
- **Knowledge Relations / Graph Preview Z0b-lite（D-121）**：新增 `knowledge_relations` 最小表、`GET/POST/PATCH/DELETE /api/relations` 和 `GET /api/graph/preview`；只允许 confirmed KU 进入手动关系和图谱预览，支持 project/folder/tag filters、duplicate/self/pending/cross-project validation；Renderer `/graph` 接入真实关系摘要、节点/边列表、关系创建/编辑/归档和 bridge degraded 状态；新增 `pnpm smoke:p0-relations-graph`。D-121 不接 GraphRAG、真实 LLM、自动关系抽取、外部图数据库或检索 ranking 写回。
...
| D-121 | Knowledge Relations / Graph Preview Z0b-lite | 新增 `knowledge_relations`、关系 CRUD API、Graph Preview API、`/graph` 手动关系创建 / 编辑 / 归档和 `smoke:p0-relations-graph` | D-120 后八页 shell 已稳定，下一步补齐 `/graph` 的真实 confirmed KU 关系预览；本阶段不接 GraphRAG、自动关系抽取、外部图数据库或 ranking 写回 | 已完成 |
