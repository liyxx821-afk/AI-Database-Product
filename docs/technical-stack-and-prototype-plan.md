...
`smoke:p0-core` 是 W1 第一阻塞门槛。D-116 后，桌面运行时改动必须先通过 `smoke:p0-desktop-runtime`，P0-File / Parser / KU / Organization / Export / Retrieval / Citation / Feedback 改动必须继续通过 `smoke:p0-file`、`smoke:p0-parse`、`smoke:p0-ku`、`smoke:p0-space-tag-metadata`、`smoke:p0-knowledge-export`、`smoke:p0-knowledge-export-history`、`smoke:p0-search-ask`、`smoke:p0-citation-detail`、`smoke:p0-citation-focus`、`smoke:p0-citation-annotate-compare`、`smoke:p0-feedback-memory`、`smoke:p0-feedback-diagnostics`、`smoke:p0-feedback-export` 和 `smoke:p0-feedback-filters-history`，再验证 P0-Z0a 的 text import / review / evidence-only 链路。
