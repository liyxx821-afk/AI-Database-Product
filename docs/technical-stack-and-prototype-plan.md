...
版本：v0.39
日期：2026-05-18
状态：已同步——完整 P0 四切片 + P0-Z0a/Z0b 竖切 + ProcessingJob + 切片前准备层 / 结构化整理检查门 + 知识切片质量闭环 + 安全运维横切层 + 技术选型矩阵 + Provider capability 契约收紧 + 切片执行 profile + AI 结构化整理 profile + D-079 存储映射 + D-080 知识调用 / implicit_agent / D-081-D085 调用 schema、反馈策略与前端系统技术矩阵 + D-088 README 对齐后的技术栈优化 + D-090 技术栈执行优化 + D-091 工程化验收门槛 + D-092 代码骨架前置契约优化 + D-093 桌面运行时硬化 + D-094 P0-Core 桌面工程骨架开工契约 + D-095 架构审查实现验收强化 + D-098 Knowledge Workspace 页面信息架构 + D-104 Search / Ask typed API 接入 + D-107 Feedback / Memory typed API 接入 + D-108 Feedback Diagnostics typed API 接入 + D-109 Desktop Runtime Smoke + D-110 Feedback Diagnostics Export typed API 接入 + D-111 Feedback Filters / Export History typed API 接入 + D-112 Citation Detail Focus typed API 接入 + D-113 Citation Annotation / Compare typed API 接入 + D-114 Organization typed API / store 接入 + D-115 Knowledge Export typed API / store 接入 + D-116 Knowledge Export History typed API / store 接入 + D-117 Packaged Runtime Static Renderer / Sidecar Artifact smoke + D-118 Batch Actions typed API / store 接入 + D-119 Text-to-SQL Preview typed API / store 接入 + D-120 UI Interaction Polish / Design-Ready Shell + D-121 Relations / Graph Preview typed API / store 接入
...
- `docs/data-model.md`：数据模型 v0.27-draft。
...
| Feedback / Memory | D-107 已实现 append-only feedback events 与 pending-review Memory Draft；D-108 已实现只读 Feedback Diagnostics / Event Replay；D-110 已实现 feedback diagnostics JSON / CSV 脱敏导出；D-111 已实现高级 filters 与 `config.json` export history metadata；Memory 仍必须进入 Review | Z0b-lite；Z2 再接 `retrieval_feedback` / provider answer |
...
`smoke:p0-core` 是 API sidecar / local token / SQLite 骨架验收；`smoke:p0-desktop-runtime` 是已 build Electron Main / Preload、renderer preview、preload bridge、受保护 API 和 sidecar clean shutdown 的 pseudo-packaged runtime 验收；`smoke:p0-packaged-runtime` 是静态 renderer dist、artifact preload、artifact sidecar、app data logs 和 clean shutdown 的安装器前 runtime 验收；`smoke:p0-i18n-settings` 是 settings / config.json language 验收；`smoke:p0-ui-polish` 是八页 route shell、i18n key parity、bridge degraded 壳层和 renderer build 验收；`smoke:p0-file` 是 upload / integrity / File Inspection Z0a 验收；`smoke:p0-parse` 是 file → Source / Chunk 验收；`smoke:p0-ku` 是 Source / Chunk → Candidate KU / Review / fallback embedding 验收；`smoke:p0-space-tag-metadata` 是 Project / Folder / Tag、Folder-Tag Mirroring、Source/KU organization 和 retrieval filters 验收；`smoke:p0-relations-graph` 是手动 confirmed KU relation、Graph Preview filters 和 archive boundary 验收；`smoke:p0-text-to-sql` 是规则模板 structured query preview、只读 SQL trace、过滤和安全边界验收；`smoke:p0-knowledge-export` 是 confirmed KU Markdown/JSON 与 Project ZIP 脱敏导出验收；`smoke:p0-knowledge-export-history` 是 knowledge export history metadata / filters replay / delete 验收；`smoke:p0-search-ask` 是 confirmed KU → Retrieval Preview / Evidence Pack detail / evidence-only ask 验收；`smoke:p0-citation-detail` 是 Evidence Pack replay / Citation Detail 验收；`smoke:p0-citation-focus` 是 Citation Detail focus / trace path / copy-safe payload 验收；`smoke:p0-citation-annotate-compare` 是 Citation annotation CRUD 与同 pack evidence compare 验收；`smoke:p0-batch-actions` 是 Source/KU batch organization、citation batch annotation 和 selected KU export 验收；`smoke:p0-feedback-memory` 是 append-only feedback event 与 Memory Draft review 验收；`smoke:p0-feedback-diagnostics` 是 feedback event replay / diagnostics summary 验收；`smoke:p0-feedback-export` 是 feedback diagnostics JSON / CSV 脱敏导出验收；`smoke:p0-feedback-filters-history` 是 feedback advanced filters 与 export history metadata 验收；`smoke:p0-z0a` 是 text import 到 evidence-only 的业务竖切验收。所有 smoke 不能混成一个脚本，且必须按顺序通过。
...
| `smoke:p0-packaged-runtime` | 静态 renderer dist、artifact preload、artifact sidecar 和 app data sidecar 日志可用；退出后 sidecar 不残留 |
| `smoke:p0-ui-polish` | route config / i18n parity / bridge degraded shell / responsive CSS markers → renderer build |
...
| `smoke:p0-space-tag-metadata` | text import → confirm KU → create folder/tag → bind Source/KU → list/retrieval filters → mirror tag / pending boundary checks |
| `smoke:p0-relations-graph` | multiple confirmed KU → create relation → graph preview filters → archive relation → invalid relation checks |
| `smoke:p0-text-to-sql` | text import → folder/tag binding → confirmed KU lookup → source inventory → evidence history → malicious query safety |
| `smoke:p0-knowledge-export` | text import → confirm KU → create folder/tag → bind KU → export filtered Markdown / JSON → export Project ZIP → no path/token leakage |
| `smoke:p0-knowledge-export-history` | text import → confirm KU → export Markdown/JSON/Project ZIP → history list/hash/filter checks → delete history → no content/path/token leakage |
...
| `smoke:p0-citation-focus` | multiple confirmed KU → Evidence Pack detail summary → focus item → trace path / copy-safe payload → invalid focus error |
| `smoke:p0-citation-annotate-compare` | multiple confirmed KU → Evidence Pack detail → annotation create/update/delete → compare 2-3 evidence items → copy-safe summary / invalid binding checks |
| `smoke:p0-batch-actions` | multiple text imports → batch Source/KU organization → retrieval filter → batch citation annotation → selected KU export → no token/path leakage |
...
| `smoke:p0-feedback-export` | evidence-only answer → multiple feedback events → export JSON → export filtered CSV → verify no token/source/path leakage |
| `smoke:p0-feedback-filters-history` | evidence-only answer → mixed feedback events → advanced filters → filtered summary/export → export history list/delete → verify no token/source/path leakage |
| `smoke:p0-z0a` | text import → rule chunk → KU review → fallback embedding → evidence-only |

`smoke:p0-core` 是 W1 第一阻塞门槛。D-121 后，桌面运行时改动必须先通过 `smoke:p0-desktop-runtime` 与 `smoke:p0-packaged-runtime`，UI shell 改动必须通过 `smoke:p0-i18n-settings` 与 `smoke:p0-ui-polish`，P0-File / Parser / KU / Organization / Relations / Text-to-SQL / Export / Retrieval / Citation / Feedback 改动必须继续通过 `smoke:p0-file`、`smoke:p0-parse`、`smoke:p0-ku`、`smoke:p0-space-tag-metadata`、`smoke:p0-relations-graph`、`smoke:p0-text-to-sql`、`smoke:p0-knowledge-export`、`smoke:p0-knowledge-export-history`、`smoke:p0-search-ask`、`smoke:p0-citation-detail`、`smoke:p0-citation-focus`、`smoke:p0-citation-annotate-compare`、`smoke:p0-batch-actions`、`smoke:p0-feedback-memory`、`smoke:p0-feedback-diagnostics`、`smoke:p0-feedback-export` 和 `smoke:p0-feedback-filters-history`，再验证 P0-Z0a 的 text import / review / evidence-only 链路。
