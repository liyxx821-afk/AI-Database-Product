...
当前阶段已从文档与架构设计进入 **P0 Knowledge Relations / Graph Preview Z0b-lite 代码阶段**。D-100 已创建 Electron + React + Vite + TypeScript、FastAPI sidecar、SQLite、OpenAPI/generated types、runtime contracts、P0-Z0a text import / review / evidence-only smoke 和八页黑白灰低保真 Knowledge Workspace 壳层；D-101 已补齐文件上传、分片接收、完整性校验、File Inspection Z0a、真实 file list 和 `smoke:p0-file`；D-102 已补齐 Parser Router Z0a、Source / Chunk 创建、parse task、source API 和 `smoke:p0-parse`；D-103 已把 parsed Source / Chunk 接入 Candidate KU、Review Queue、384 维 fallback embedding 和 `smoke:p0-ku`；D-104 已把 confirmed KU 接入 Retrieval Preview、Evidence Pack detail、Search / Ask 真实查询状态和 `smoke:p0-search-ask`；D-105 已把 Evidence Pack detail 扩展为可复盘 Citation Detail，并在 `/search`、`/ask` 内嵌 detail replay 面板和 `smoke:p0-citation-detail`；D-106 已把八页工作台 UI 文案改为默认中文、可切换英文，并通过 `/api/settings` 将语言偏好持久化到 app data 的 `config.json`；D-107 已新增 append-only feedback events、pending-review Memory Draft、memory review confirm/ignore、`/ask` 反馈与保存入口、`/outputs` 草稿摘要和 `smoke:p0-feedback-memory`；D-108 已新增 Feedback Diagnostics 只读列表 / 聚合摘要 API、`/outputs` 反馈诊断面板和 `smoke:p0-feedback-diagnostics`；D-109 已新增 `smoke:p0-desktop-runtime`，验证已 build Electron Main / Preload、renderer preview、FastAPI sidecar、preload bridge、local token、受保护 API、非空 Renderer 和 sidecar clean shutdown；D-110 已新增 Feedback Diagnostics JSON / CSV 脱敏导出 API、`/outputs` 当前筛选导出按钮和 `smoke:p0-feedback-export`；D-111 已新增 feedback diagnostics 高级过滤、`config.json.feedback_export_history` 最近 20 条导出历史 metadata、`/outputs` Export History 面板和 `smoke:p0-feedback-filters-history`；D-112 已将 Citation Detail 扩展为可聚焦、可筛选、可排序、可复制引用轨迹的 Evidence Trace 复盘面板，并新增 `smoke:p0-citation-focus`；D-113 已新增持久化 citation annotations、同一 Evidence Pack 内 evidence compare API、Citation Detail 批注 / 对比面板和 `smoke:p0-citation-annotate-compare`；D-114 已补齐 Project / Folder / Tag API、Folder-Tag Mirroring、Source/KU 组织绑定、检索过滤、`/library` Organization 面板、`/search` / `/ask` 组织过滤条和 `smoke:p0-space-tag-metadata`；D-115 已新增 KU Markdown/JSON 导出、Project ZIP 导出、`/outputs` Knowledge Export 面板和 `smoke:p0-knowledge-export`；D-116 已新增 `config.json.knowledge_export_history` 最近 20 条知识导出历史 metadata、`GET/DELETE /api/exports/history`、`/outputs` Knowledge Export History 面板和 `smoke:p0-knowledge-export-history`；D-117 已新增 `smoke:p0-packaged-runtime`，验证 Electron 加载静态 renderer dist、preload / sidecar artifact 路径、file/hash route、local token auth、app data sidecar 日志和 clean shutdown；D-118 已新增批量 Source/KU organization API、Citation 批量批注 API、`/library` 批量绑定、`/outputs` KU 选择导出、Citation Detail 批量批注选择和 `smoke:p0-batch-actions`；D-119 已新增规则模板版 `/api/text-to-sql/preview`、`/search` SQL Trace 面板和 `smoke:p0-text-to-sql`；D-120 已补齐八页工作台交互壳层 polish、统一 route 摘要 / bridge degraded 横幅 / 状态 chip / 响应式布局约束和 `smoke:p0-ui-polish`；D-121 已新增手动 confirmed KU 关系、`/api/relations`、`/api/graph/preview`、`/graph` 真实关系预览 / 创建 / 编辑 / 归档面板和 `smoke:p0-relations-graph`。
...
- [docs/data-model.md](./docs/data-model.md)：数据模型 v0.27-draft，定义账号预埋、Project / Folder / Tag、Folder-Tag Mirroring、Source/KU organization 绑定、上传、文件、File Inspection、解析、切片前准备层、检查门映射、知识切片质量、切片执行 profile、AI 结构化整理 profile、D-079 结构化整理子字段与存储映射、D-080 调用 profile 元数据、D-081/D-082/D-083/D-085 调用边界与 profile schema、D-092 trace chain 与 Evidence Pack 失败态、D-104 Retrieval Preview 复用既有 `retrieval_logs` / `evidence_packs` / `evidence_items` / `ai_answers` 边界、D-107/D-108 `feedback_events` / `memories` 与只读诊断边界、D-113 `citation_annotations` 批注边界、D-114 `source_tags` / `knowledge_unit_tags` 组织过滤边界、D-115/D-116 知识资产导出 envelope / ZIP manifest / export history metadata 边界、安全运维事件、事件枚举单一来源、知识单元、向量、RAG answer、用户记录对象、Provider capability/fallback 元数据和 P0-Z0a/Z0b/Z1/Z2 迁移波次。
...
- [docs/api-design.md](./docs/api-design.md)：P0 API 设计草案 v0.32，定义 Auth preembed、Project / Folder / Tag、Folder-Tag Mirroring、Source/KU organization 绑定、Upload、File Inspection、ProcessingJob Events、File processing、Knowledge pipeline、chunk build、KU extract structuring summary、切片前准备、检查门摘要、切片执行 profile summary、Retrieval Preview、Text-to-SQL Preview、Evidence Pack detail / Citation Detail replay、RAG answer/fallback、query understanding、retrieval strategy、ranking、citation trace、feedback actions / feedback policy、Provider capability status、桌面系统 API、D-106 `/api/settings` language 持久化边界、D-107 `/api/feedback` / `/api/memory-drafts` 回流边界、D-108 `GET /api/feedback` / `GET /api/feedback/summary` 诊断读取边界、D-110 `GET /api/feedback/export` JSON / CSV 脱敏导出边界、D-111 高级 filters / export history 边界、D-113 citation annotation / evidence compare 边界、D-114 organization filters 边界、D-115/D-116 `/api/exports/knowledge-units` / `/api/exports/project` / `/api/exports/history` 知识资产导出和历史复盘边界、D-119 `/api/text-to-sql/preview` 规则模板查询预览边界，以及 D-121 `/api/relations` / `/api/graph/preview` 手动关系与图谱预览边界。
- [docs/text-to-sql.md](./docs/text-to-sql.md)：Text-to-SQL P0 查询契约 v0.5，定义白名单对象、只读视图、典型 SQL 模板、权限过滤、Query Explanation、D-081/D-085 的 Z0a retrieval_log 锚点 / Z2 persisted 调用对象边界，以及 D-119 规则模板版 structured query preview。
- [docs/api-implementation-plan.md](./docs/api-implementation-plan.md)：API route-level 实施计划 v0.33-draft，定义 P0-Core / P0-File / P0-AI / P0-RAG 的 route、DTO、service、repository、事务边界、job-first chunk build、切片前准备、切片执行 profile、AI 结构化整理 profile、知识调用 profile、隐式单 Agent 边界、D-081-D085 调用边界、D-092 OpenAPI 类型生成 / trace chain / migration 波次命名、D-093 桌面运行时 API 约束、D-094 P0-Core API skeleton 顺序、D-098 页面到 API service 映射、D-105 Citation Detail / Evidence Pack replay 实现边界、D-106 SettingsService / config.json language 持久化边界、D-107 FeedbackService / MemoryService Z0b-lite 实现边界、D-108 Feedback Diagnostics 只读诊断边界、D-110 Feedback Diagnostics Export 只读脱敏导出边界、D-111 advanced filters / export history config 持久化边界、D-113 CitationAnnotationService / Evidence compare 边界、D-114 OrganizationService / Folder-Tag / metadata filters 边界、D-115/D-116 KnowledgeExportService / Project ZIP / export history 边界、D-119 TextToSqlTemplateService preview、知识切片质量闭环、安全运维状态、P0-Z0a/Z0b 竖切和 Repository 抽象层接口契约。
- [docs/desktop-architecture.md](./docs/desktop-architecture.md)：桌面应用架构 v0.12，定义 Electron 选型、FastAPI sidecar 部署、SQLite 本地数据库、IPC 通信、打包分发、文件系统集成、离线边界、桌面 UX 约束、数据可移植性、Schema 演进与数据迁移工具链、Electron 安全实践、产品包装规范、首次体验流程、D-092 sidecar 打包验证 spike、D-093 桌面运行时硬化、D-094 P0-Core 启动状态机 / preload API surface、D-098 桌面 Shell / 八页内部路由、D-106 语言设置持久化、D-109 pseudo-packaged desktop runtime smoke、D-111 feedback export history metadata config、D-116 knowledge export history metadata config 和 D-117 packaged runtime static renderer / sidecar artifact smoke。
...
- [docs/testing-strategy.md](./docs/testing-strategy.md)：测试与评估策略 v0.38，定义测试金字塔、P0-Z0a fixture、ProcessingJob fixture、File Inspection fixture、切片前准备 fixture、检查门映射 fixture、Chunk Quality fixture、AI 结构化整理 fixture、知识调用路由 fixture、D-081-D085 持久化边界/profile/前端状态 fixture、D-098 页面契约 fixture、D-105 Citation Detail smoke、D-106 i18n settings smoke、D-107 feedback / memory smoke、D-108 feedback diagnostics smoke、D-109 desktop runtime smoke、D-110 feedback export smoke、D-111 feedback filters / history smoke、D-112 citation detail focus smoke、D-113 citation annotation / compare smoke、D-114 space/tag/metadata smoke、D-115 knowledge export smoke、D-116 knowledge export history smoke、D-117 packaged runtime smoke、D-118 batch actions smoke、D-119 text-to-sql preview smoke、D-120 UI polish smoke、D-121 relations graph smoke、安全运维 fixture、D-090 技术栈执行优化 fixture、D-091 工程化验收门槛、D-092 代码骨架前置契约测试口径、D-093 桌面运行时硬化测试口径、D-094 P0-Core 工程骨架开工测试口径、SQLite 并发测试、Electron + Playwright E2E 方案、CI 集成预案和 P1 AI 能力评估方案。
...
- [docs/technical-stack-and-prototype-plan.md](./docs/technical-stack-and-prototype-plan.md)：技术栈与 P0 原型实施计划 v0.39，推荐 Electron + React + Vite + TypeScript、FastAPI sidecar、SQLite + sqlite-vec，并根据 README 入口口径把技术栈优化为 P0-Z0a 最小可执行栈、完整 P0 扩展栈和 P1/P2 后置栈，同时落地 D-090 依赖分组、Zustand 状态、sqlite-vec capability probe、File Inspection 分层、evidence-first RAG 执行口径、D-091 工程化验收门槛、D-092 代码骨架前置契约、D-093 桌面运行时硬化、D-094 P0-Core 桌面工程骨架开工契约、D-095 架构审查实现验收强化、D-098 Knowledge Workspace 页面 IA、D-104 Search / Ask typed API 接入、D-107 Feedback / Memory typed API 接入、D-108 Feedback Diagnostics typed API 接入、D-109 Desktop Runtime Smoke、D-110 Feedback Diagnostics Export、D-111 Feedback Filters / Export History typed API 接入、D-113 Citation Annotation / Compare typed API 接入、D-114 Organization typed API / store 接入、D-115/D-116 Knowledge Export typed API / history store 接入、D-117 packaged runtime artifact smoke、D-118 Batch Actions typed API / store 接入、D-119 Text-to-SQL Preview typed API / store 接入、D-120 UI Interaction Polish / Design-Ready Shell 和 D-121 Relations / Graph Preview typed API / store 接入。
...
P0 技术栈已确定（详见 `docs/development-plan.md` D-029 / D-039 / D-040），当前已进入 Knowledge Relations / Graph Preview Z0b-lite 代码阶段。
...
- `pnpm smoke:p0-packaged-runtime`
- `pnpm smoke:p0-i18n-settings`
- `pnpm smoke:p0-ui-polish`
- `pnpm smoke:p0-file`
- `pnpm smoke:p0-parse`
- `pnpm smoke:p0-ku`
- `pnpm smoke:p0-space-tag-metadata`
- `pnpm smoke:p0-relations-graph`
- `pnpm smoke:p0-text-to-sql`
- `pnpm smoke:p0-knowledge-export`
- `pnpm smoke:p0-knowledge-export-history`
- `pnpm smoke:p0-search-ask`
- `pnpm smoke:p0-citation-detail`
- `pnpm smoke:p0-citation-focus`
- `pnpm smoke:p0-citation-annotate-compare`
...
- 已完成 UI Interaction Polish / Design-Ready Shell Z0b-lite（D-120）：
  - Renderer 八页工作台新增 route summary、普通浏览器 bridge degraded 横幅、统一 state chip 文案和更稳定的黑白灰低保真响应式布局。
  - `/search`、`/ask`、`/library`、`/outputs` 的高密度面板继续复用现有功能边界，仅改善工具条、状态、筛选、列表和窄屏排版，不改 API / 数据模型 / 主路由。
  - 新增 `pnpm smoke:p0-ui-polish`，检查 8 页路由、i18n key 对齐、关键 shell/CSS marker 与 renderer build。

- 已完成 Text-to-SQL Template / Structured Query Preview Z0b-lite（D-119）：
  - 新增 `POST /api/text-to-sql/preview`，以 confirmed KU lookup、source inventory、evidence history 三类只读 SELECT 模板返回 SQL trace、参数、结果行、query explanation 和 degraded provider/fallback 状态。
  - Renderer `/search` 新增结构化查询预览 / SQL Trace 面板，复用 Project / Folder / Tag filters，通过 preload bridge + typed fetch wrapper 调用后端；普通浏览器继续显示 bridge degraded。
  - 新增 `pnpm smoke:p0-text-to-sql`；本阶段不接真实 Text-to-SQL provider、不开放 SQL 编辑器、不修改 retrieval ranking 或 Evidence Pack 组装。

- 已完成 Citation / Organization / Export Batch Actions Z0b-lite（D-118）：
  - 新增 `PATCH /api/sources/organization:batch` 与 `PATCH /api/knowledge-units/organization:batch`，批量复用现有 folder/tag 替换语义；Source 批量绑定继续同步派生 KU 的 folder mirror 与 source-assignment tags。
  - 新增 `POST /api/evidence-packs/{id}/annotations:batch`，为同一 Evidence Pack 内 1-20 个 evidence item 创建同内容本地 citation 批注；不写 feedback、不改 ranking。
  - Renderer `/library` 增加 Source/KU 多选批量绑定，Citation Detail 增加批量批注选择，`/outputs` Knowledge Export 增加当前过滤 KU 多选导出；新增 `pnpm smoke:p0-batch-actions`。

- 已完成 Packaged Runtime Static Renderer / Sidecar Artifact Spike Z0b-lite（D-117）：
  - Electron Main 增加 renderer / preload / sidecar artifact 路径解析，D-117 smoke 可直接加载 build 后的静态 `renderer/dist/index.html`，不再依赖 Vite preview。
  - Renderer 增加 file/hash route 兼容，静态 Electron 模式首屏保持 `/dashboard`，普通浏览器和 Vite dev 继续使用 pathname route。
  - SidecarManager 支持 `KB_SIDECAR_APP_DIR` 与 `KB_SIDECAR_LOG_DIR`，sidecar 从 artifact API 目录启动并把 stdout/stderr 写入 app data logs；local token 仍只通过 preload bridge 进入 typed fetch wrapper。
  - 新增 `pnpm smoke:p0-packaged-runtime`；本阶段不做正式签名、自动更新、真实安装器、正式品牌资源或 PyInstaller 完整产物。

- 已完成 Knowledge Export History / Replay Z0b-lite（D-116）：
  - 新增 `GET /api/exports/history` 与 `DELETE /api/exports/history/{id}`；每次知识导出后向 `config.json.knowledge_export_history` 写入最近 20 条脱敏 metadata。
  - History 只保存 export kind、filename、format、record_count、generated_at、filters、summary、content_sha256 和 redaction flags，不保存 Markdown / JSON / ZIP 正文。
  - Renderer `/outputs` Knowledge Export 面板新增 Knowledge Export History，支持刷新、删除记录和套用历史过滤条件。
  - 新增 `pnpm smoke:p0-knowledge-export-history`；仍不做云同步、备份/还原、长期导出文件存储、Obsidian 双向同步、真实 LLM 或 GraphRAG。

- 已完成 Knowledge Unit / Project Export Z0b-lite（D-115）：
  - 新增 `POST /api/exports/knowledge-units`：支持 Markdown / JSON、project / folder / tag / KU ID 过滤、include chunks / sources 和显式 pending review 导出。
  - 新增 `POST /api/exports/project`：返回最小 Project ZIP envelope，包含 `manifest.json`、`knowledge-units.json`、`tags.json`、`folders.json`、`sources.json`、`chunks.json` 与 `README.md`。
  - Renderer `/outputs` 增加 Knowledge Export 面板，复用 Organization filters，使用 Blob download；普通浏览器无 bridge 时保持 degraded。
  - 新增 `pnpm smoke:p0-knowledge-export`；云同步、备份/还原、系统级诊断包、Obsidian 双向同步、真实 LLM、provider-backed RAG 和 GraphRAG 继续后置。

- 已完成 Knowledge Space / Folder-Tag / Metadata Filters Z0b-lite（D-114）：
  - 新增 Project / Folder / Tag API、Folder-Tag Mirroring、`source_tags` / `knowledge_unit_tags` join 表和 Source/KU organization patch API。
  - `GET /api/sources`、`GET /api/knowledge-units`、`POST /api/retrieval/preview`、`POST /api/retrieval/evidence-only` 已支持 `project_id`、`folder_id`、`tag_ids` 过滤，且仍只检索 confirmed KU。
  - Renderer `/library` 增加 Organization 面板，`/search` 与 `/ask` 增加组织过滤条，`/dashboard` 显示 folder/tag 计数；普通浏览器无 bridge 时保持 degraded。
  - 已新增 `pnpm smoke:p0-space-tag-metadata`；任意 metadata JSON 编辑器、关系图谱、真实 LLM、provider-backed RAG、reranker 和 GraphRAG 继续后置。

- 已完成 Citation Annotation / Evidence Compare Z0b-lite（D-113）：
  - 新增 `citation_annotations` 最小表和批注 CRUD API，批注绑定同一 Evidence Pack 内的 evidence item，支持 `note / question / risk / follow_up`。
  - 新增 `POST /api/evidence-packs/{id}/compare`，只读对比同一 Evidence Pack 内 2-3 条 evidence items，返回 rank、source、chunk、KU、trace path 和 copy-safe summary。
  - Renderer `/search` 与 `/ask` 的 Citation Detail 面板新增 focused item 批注区、编辑 / 删除、compare 选择、对比结果和复制摘要；不新增主路由。
  - 已新增 `pnpm smoke:p0-citation-annotate-compare`；真实 LLM、provider-backed RAG、reranker、GraphRAG、feedback 写回、compare 持久化和复制行为记录继续后置。
