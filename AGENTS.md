...
- 每轮实际优化、代码修改、架构文档修改或计划文档修改完成并验证后，Codex 应将本轮相关变更提交并推送到 GitHub。
- 该规则只适用于实际产生文件变更的任务；只读分析、讨论、review 或用户明确要求“不提交”的任务不自动提交。
- 提交前必须检查 `git status` 与 diff，区分本轮变更和已有未提交变更；工作区混杂时，只 stage 本轮明确相关文件，不把无关改动打包进 commit。
- 提交前应运行与本轮范围匹配的验证；验证失败时不要推送，除非用户明确要求保留失败状态并提交。
- 默认在当前分支提交并推送，不主动新建分支；如果远端保护、权限或冲突导致无法推送，应停止并报告具体阻塞，不强推、不绕过保护规则。
- 不提交 `.env`、密钥、私人数据、本地数据库、缓存、生成日志或大体积运行产物。
